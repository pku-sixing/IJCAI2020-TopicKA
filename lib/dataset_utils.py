...
from KEFU3 import kefu_knowledge_utils as kefu_knowledge_utils3


def create_flexka3_iterator(hparams, is_eval=False):
    tf_entity_embed, tf_entity_fact_embedding,\
    tf_entity_source, tf_entity_target, entity_vocab, \
    reverse_entity_vocab, word2entity, entiy2words, \
    tf_fact_idf = kefu_knowledge_utils3.load_entity_vocab(
        hparams)

    iterator, skip_count_placeholder\
        , src_file_placeholder, tgt_in_file_placeholder,\
         tgt_out_file_placeholder, fact_file_placeholder, cue_fact_placeholder, neg_fact_placeholder,\
          src_vocab_table, tgt_vocab_table\
        , reverse_tgt_vocab_table = kefu_knowledge_utils3.create_kefu_iterator_from_file(hparams, is_eval=is_eval)

    inputs = dict()
    inputs['word2entity'] = word2entity
    inputs['entity2word'] = entiy2words
    ...
    inputs['golden_facts'] = iterator.golden_facts
    inputs['neg_fact'] = iterator.neg_fact
    inputs['neg10_fact'] = iterator.neg10_fact
    inputs['neg10_fact_length'] = iterator.neg10_len
    inputs['embedding_vocab'] = tf.get_variable('embedding_encoder', shape=[hparams['src_vocab_size'], hparams['embed_dim']])
    ...
    def init_func(sess, prefix=""):
        ...
        sess.run(iterator.initializer, feed_dict={skip_count_placeholder: 0,
                                              ...
                                              tgt_out_file_placeholder: hparams['%stgt_file' % prefix]+tgt_out_suffix,
