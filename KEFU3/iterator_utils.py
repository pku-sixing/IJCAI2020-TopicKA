...
class BatchedInput(
    collections.namedtuple("BatchedInput",
                           ("initializer", "source", "target_input", "fact", "fact_length",
                            ...
                            "cue_fact", "golden_facts", "neg_fact", "neg10_fact","neg10_len"
                            ))):
    pass
...
def get_iterator(src_dataset,
  ...
  ent_unk_id = tf.cast(vocab_table.lookup(tf.constant(knowledge_utils.UNK_ENTITY)), tf.int32)
  ...
  # Select One Neg Fact
  src_tgt_dataset = src_tgt_dataset.map(
      lambda src, tgt_in, tgt_out, fact, cue, neg: (src, tgt_in, tgt_out, fact,
                                                    tf.random_shuffle(cue)[:1],
                                                    cue,
                                                    tf.random_shuffle(neg)[:tf.minimum(tf.size(cue), tf.size(neg))],
                                                    tf.random_shuffle(neg)[:tf.minimum(tf.size(cue) * 10, tf.size(neg))]),
      num_parallel_calls=num_parallel_calls).prefetch(output_buffer_size)


  # Filter zero length input sequences.
  src_tgt_dataset = src_tgt_dataset.filter(
      lambda src, tgt_in, tgt_out, fact, cue, golden, neg, neg10: tf.logical_and(tf.size(src) > 0, tf.size(tgt_in) > 0))


  if src_max_len:
    src_tgt_dataset = src_tgt_dataset.map(
        lambda src, tgt_in, tgt_out, fact, cue, golden, neg,neg10: (src[:src_max_len], tgt_in, tgt_out, fact, cue, golden, neg,neg10),
        num_parallel_calls=num_parallel_calls).prefetch(output_buffer_size)
  if tgt_max_len:
    src_tgt_dataset = src_tgt_dataset.map(
        lambda src, tgt_in, tgt_out, fact, cue, golden, neg,neg10: (src, tgt_in[:tgt_max_len], tgt_out[:tgt_max_len], fact, cue, golden, neg,neg10),
        num_parallel_calls=num_parallel_calls).prefetch(output_buffer_size)

  # Convert the word strings to ids.  Word strings that are not in the
  # vocab get the lookup table's default_value integer.
  src_tgt_dataset = src_tgt_dataset.map(
        lambda src, tgt_in, tgt_out, fact, cue, golden, neg,neg10: (
                          ...
                          tf.string_to_number(golden, tf.int32),
                          tf.string_to_number(neg, tf.int32),
                          tf.string_to_number(neg10, tf.int32),
                         ),
  ...
  src_tgt_dataset = src_tgt_dataset.map(
      lambda src, tgt_in, tgt_out, entity_src, entity_tgt_in, fact, cue, golden, neg,neg10: (
                            src,
  ...
  src_tgt_dataset = src_tgt_dataset.map(
        lambda src, tgt_in, tgt_out, entity_src, entity_tgt_in,fact, cue, golden, neg,neg10: (
            src,  tgt_in, tgt_out,entity_src, entity_tgt_in, fact, cue, golden, neg,neg10, tf.div(tf.size(src), sub_token_num), tf.size(tgt_in), tf.size(fact), tf.size(neg10)),
        num_parallel_calls=num_parallel_calls)
  ...
  def batching_func(x):
    return x.padded_batch(
        ...
        padded_shapes=(
            ...
            tf.TensorShape([None]),  # golden
            tf.TensorShape([None]),  # neg
            tf.TensorShape([None]),  # neg10
            tf.TensorShape([]),  # src_len
            tf.TensorShape([]),  # tgt_len
            tf.TensorShape([]),  # fact_len
            tf.TensorShape([]),  # neg_len
  ...
  if num_buckets > 1:
    ...
    def key_func(unused_1, unused_2, unused_3,unused_4, entity_src, entity_tgt_in, cue, golden, neg,neg_10, src_len, tgt_len, fact_len, neg10_len):
  ...
  (src_ids, tgt_input_ids, tgt_output_ids,entity_src_ids, entity_tgt_in_ids, fact_ids, cue_id, golden_id, neg_id, neg10_ids,
   src_seq_len,tgt_seq_len, fact_len, neg10_len) = (batched_iter.get_next())
  return BatchedInput(
      ...
      cue_fact=cue_id,
      golden_facts=golden_id,
      neg_fact=neg_id,
      neg10_fact=neg10_ids,
      neg10_len=neg10_len,
  )
