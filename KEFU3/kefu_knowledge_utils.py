...
from KEFU3 import iterator_utils
...
def load_entity_vocab(hparams):
    ...
    with open(entity2word_dict_path, encoding='utf-8') as fin:
        ...
        entiy2words = tf.get_variable('entity2words', dtype=tf.int32, initializer=entiy2words, trainable=False)
    ...
    tf_entity_embed = tf.layers.dense(tf_entity_embed, hparams['entity_dim'], use_bias=False, name='entity_embedding_transformer')
    tf_relation_embed = tf.layers.dense(tf_relation_embed, hparams['entity_dim'], use_bias=False,  name='relation_embedding_transformer')

    if inv_relation:
        print('inv_relation')
        inv_relation = tf.layers.dense(relation_embed, hparams['entity_dim'], use_bias=False,
                                       name='inv_relation_embedding_transformer')
        tf_relation_embed = tf.concat([tf_relation_embed, inv_relation], axis=0)
    ...
    with open(fact_dict_path, encoding='utf-8') as fin:
        ...
        for line in lines:
            ...
            items[3] = int(relation_dict.get(items[3])) + len(entity_dict) # realtion和 entity共用一个列表
