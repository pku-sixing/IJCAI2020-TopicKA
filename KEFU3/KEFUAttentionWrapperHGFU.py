...
class AttentionWrapper(rnn_cell_impl.RNNCell):
  ...
  """
    ...
    def __init__(self,
                 ...
                 cue_cell,
                 cue_inputs,
                 fact_candidates,# embedding
                 lengths_for_fact_candidates,
                 kgp_initial_goals,
                 attention_mechanism,
                 enable_hgfu=False,
                 cue_inputs_usage=None,
                 fact_attention_mode=None,
                 encoder_memory=None,
    ...
    **NOTE** If you are using the `BeamSearchDecoder` with a cell wrapped in
    ...
    """
        ...
        self._fact_candidates_projection = tf.layers.dense(self._fact_candidates, units=sim_dim,
                                                           activation=tf.nn.elu,
                                                           name='entity_key_projection')

        self._lengths_for_fanct_candidates = lengths_for_fact_candidates
        self._batch_size = tf.shape(cue_inputs)[0]
        self._cue_inputs_usage = [] if cue_inputs_usage is None else cue_inputs_usage
        self._fact_attention_mode = [] if fact_attention_mode is None else fact_attention_mode
        ...
        if self._cue_fact_mode is False:
            self._cue_inputs = tf.zeros([self._batch_size, 1])
    ...
    """
        max_candidate_num = tf.shape(self._fact_candidates)[1]
    ...
    **NOTE** Please see the initializer documentation for details of how
    ...
    """
        ...
        with tf.variable_scope("background_knowledge_attention"):
            if 'cue_fact' in self._fact_attention_mode:
                fact_attention_query = tf.layers.dense(tf.concat([inputs,self._cue_inputs],-1), units=self._sim_vec_dim,
                                                       activation=tf.nn.elu,
                                                       name='fact_query_projection_with_cue')
            else:
                fact_attention_query = tf.layers.dense(inputs, units=self._sim_vec_dim,
                                                       activation=tf.nn.elu,
                                                       name='fact_query_projection')
            fact_attention_keys = self._fact_candidates_projection
            fact_attention_query = tf.expand_dims(fact_attention_query, 1)
            fact_attention_query = tf.tile(fact_attention_query, [1, maximium_candidate_num, 1])
            fact_attention_scores = tf.reduce_sum(fact_attention_query * fact_attention_keys, -1)
            fact_attention_mask = tf.sequence_mask(self._lengths_for_fanct_candidates, dtype=tf.float32)
            fact_attention_mask = (1.0 - fact_attention_mask) * -1e10
            # entity_update_mask = tf.expand_dims(entity_update_mask, -1)
            fact_attention_scores += fact_attention_mask
            fact_attention_probs = tf.nn.softmax(fact_attention_scores, -1)
            fact_attention_probs = tf.expand_dims(fact_attention_probs, -1)
            fact_attention = fact_embedding * fact_attention_probs
            fact_attention = tf.reduce_sum(fact_attention, 1)

        # [batch, dim]

        if self._enable_hgfu:
            ...
            with tf.variable_scope('cue_gru'):
                cue_cell_inputs = tf.concat([self._cell_input_fn(inputs, state.attention), self._cue_inputs, fact_attention], -1)
                outputs_cue, cue_cell_state = self._cue_cell(cue_cell_inputs, cell_state)
                ...
                k = tf.layers.dense(tf.concat([transformed_hy, transformed_hw], -1), units=self._cue_cell.state_size,
            ...
            std_cell_inputs = tf.concat([self._cell_input_fn(inputs, state.attention), self._cue_inputs, fact_attention], -1)
            outputs_std, cell_state = self._std_cell(std_cell_inputs, cell_state)
            cell_output = outputs_std
            next_cell_state = cell_state

        dynamic_inputs_1 = tf.concat([cell_output, inputs], -1)

        if 'entity_prediction' in self._cue_inputs_usage:
            dynamic_inputs_1 = tf.concat([dynamic_inputs_1, self._cue_inputs], -1)

        entity_update_scores_p1 = tf.layers.dense(dynamic_inputs_1, units=self._sim_vec_dim,
                                                  activation=tf.nn.elu,
                                                  name='entity_query_projection')
        entity_update_scores_p2 = self._fact_candidates_projection
        ...
        if self._entity_predict_mode or self._copy_predict_mode:
            ...
            if self._copy_predict_mode:
                ...
                copy_layer1_p1 = tf.layers.dense(copy_input_p1, units=self._sim_vec_dim, activation=tf.nn.elu,
                                                 name='copy_query')
            ...
            if self._entity_predict_mode:
                ...
                if self._kg_initial_goals is not None:
                    if self.balance_gate is True:
                        ...
                        balance_selector = tf.layers.dense(balance_selector_input, 1, activation=tf.nn.sigmoid,
                                                           name='entity_balance_selector')
                        entity_probs = tf.nn.softmax(entity_logits, -1) * balance_selector + self._kg_initial_goals * (
                                    1.0 - balance_selector)
            ...
            layer1 = tf.layers.dense(mode_selector_input, self._sim_vec_dim, use_bias=True, activation=tf.nn.elu,
                                     name='selector_l1')

            common_selector = tf.layers.dense(layer1, 1, use_bias=False, name='common_selector') + (
                        (1.0 - selector_mask[0]) * -1e10)
            copy_selector = tf.layers.dense(layer1, 1, use_bias=False, name='copy_selector') + (
                        (1.0 - selector_mask[1]) * -1e10)
            entity_selector = tf.layers.dense(layer1, 1, use_bias=False, name='entity_selector') + (
                        (1.0 - selector_mask[2]) * -1e10)
            ...
            if self._binary_decoding:
                new_common_selector = tf.where(tf.greater_equal(common_selector, entity_selector),
                                               common_selector + entity_selector, tf.zeros_like(common_selector))
                new_entity_selector = tf.where(tf.greater(entity_selector, common_selector),
                                               common_selector + entity_selector, tf.zeros_like(entity_selector))
                ...
                # copy_alignments = copy_alignments.write(state.time, copy_probs)
                # fact_alignments = fact_alignments.write(state.time, entity_probs)
