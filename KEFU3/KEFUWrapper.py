...
class KEFUWrapper(tf.nn.rnn_cell.RNNCell):
    ...
    def __init__(self, cell, cell_ke, std_input_dim, entity_dim, input_entity_ids, cue_fact_embedding, cue_fact_mode=False, force_mode=False, initial_cell_state=None, name=None, export_gate_history=False):
      """
      Args:
          cell:
          encoder_states:
    ...
    def __call__(self, inputs, state, scope=None):
      ...
      # [batch, embed]
      # k = tf.layers.dense(tf.concat([transformed_hy, transformed_hw, cell_state], -1), units=self._cell.state_size,
      #                     activation=tf.nn.relu, name='FusionGate_k1')
      # k = tf.layers.dense(k, units=self._cell.state_size,
      #                     activation=tf.sigmoid, name='FusionGate_k2')
      #
      # fusion_gate = fusion_gate.write(time_step, tf.reduce_mean(k, -1))
      transformed_vy = tf.layers.dense(std_cell_state, units=self._cell.state_size,name='FusionGate_VX')
      transformed_vw = tf.layers.dense(ke_cell_state, units=self._cell.state_size,name='FusionGate_VW')
      ...
      outputs = cell_state
      #
      # cell_state = std_cell_state + ke_cell_state
      # outputs = cell_state
